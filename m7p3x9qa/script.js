...
setText('about-text', resumeData.summary);
...
resumeData.highlights.forEach((project) => {
  const card = document.createElement('article');
  card.className = 'proj';
  card.innerHTML = `
    <h3>${project.title}</h3>
    <div class="meta">${project.tech_stack}</div>
    <ul>${project.bullets.map((bullet) => `<li>${bullet}</li>`).join('')}</ul>
    <div class="tags">${project.tech_stack.split('·').map((tag) => `<span class="tag">${tag.trim()}</span>`).join('')}</div>
  `;
